...
use tokio_rustls::rustls::{self, pki_types::{CertificateDer, PrivateKeyDer}, ServerConfig};
...
impl SingleCerts {
    pub fn new(certs_path: &Path, keys_path: &Path) -> io::Result<Self> {
        Ok(Self {
            certs: certs(&mut BufReader::new(File::open(certs_path)?)).collect::<io::Result<Vec<CertificateDer<'static>>>>()?,
